package fr.adaming.controller;

import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;

@Controller
public class loginController {

	@RequestMapping(value = "/loginGerant")
	public String pageLoginGerant(){
		return "loginPagesGerant";
	}
	
	@RequestMapping(value ="/loginConseiller")
	public String pageLoginConseiller(){
		return "loginPagesConseiller";
	}
	
	@RequestMapping(value ="/logoutConseiller")
	public String pageLogoutConseiller(){
		return "index";
	}
	
	@RequestMapping(value ="/logoutGerant")
	public String pageLogoutGerant(){
		return "index";
	}
}
